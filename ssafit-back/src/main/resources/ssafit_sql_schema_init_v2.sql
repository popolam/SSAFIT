...
-- Schema ssafit_db_v2
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `ssafit_db_v2` ;
CREATE SCHEMA `ssafit_db_v2` DEFAULT CHARACTER SET utf8 ;
USE `ssafit_db_v2` ;

-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`channel`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`channel`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`channel` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`video`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`video`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`video` (
  ...
  CONSTRAINT `fk_video_channel1`
    ...
    REFERENCES `ssafit_db_v2`.`channel` (`channel_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`tag`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`tag`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`tag` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`tag_has_video`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`tag_has_video`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`tag_has_video` (
  `tag_name` VARCHAR(100) NOT NULL,
  `video_id` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`tag_name`, `video_id`),
  INDEX `fk_tag_has_video_video1_idx` (`video_id` ASC) VISIBLE,
  INDEX `fk_tag_has_video_tag1_idx` (`tag_name` ASC) VISIBLE,
  CONSTRAINT `fk_video_has_tag`
    FOREIGN KEY (`tag_name`)
    REFERENCES `ssafit_db_v2`.`tag` (`tag_name`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_tag_has_video`
    FOREIGN KEY (`video_id`)
    REFERENCES `ssafit_db_v2`.`video` (`video_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`user`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`user`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`user` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`mark`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`mark`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`mark` (
  `user_id` VARCHAR(45) NOT NULL,
  `video_id` VARCHAR(45) NOT NULL,
  ...
  CONSTRAINT `fk_mark_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `ssafit_db_v2`.`user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`follow`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`follow`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`follow` (
  `user_id` VARCHAR(45) NOT NULL,
  `channel_id` VARCHAR(45) NOT NULL,
  ...
  CONSTRAINT `fk_user_has_channel`
    FOREIGN KEY (`channel_id`)
    REFERENCES `ssafit_db_v2`.`channel` (`channel_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_channel_has_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `ssafit_db_v2`.`user` (`user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
-- -----------------------------------------------------
-- Table `ssafit_db_v2`.`review`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v2`.`review`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v2`.`review` (
...
SELECT * FROM `ssafit_db_v2`.`channel`;
...
SELECT `video`.`video_id`, `video`.`title`, `channel`.`channel_name` , `video`.`view_cnt`
	FROM `ssafit_db_v2`.`video`
        LEFT OUTER JOIN `channel` ON `video`.`channel_id` = `channel`.`channel_id`;
        
        
-- -----------------------------------------------------
-- Table `youtube_db`.`user`
-- -----------------------------------------------------
INSERT
	INTO `ssafit_db_v2`.`user` 
		(`user_id`, `nickname`, `password`, `name`, `age`, `grade`)
	VALUES 
		('admin', '관리자', 'admin', '관리자', 0, 0),
		('user1', 'nickname1', '1111', '성1 이름1', 21, 1),
        ('user2', 'nickname2', '2222', '성2 이름2', 22, 1),
        ('user3', 'nickname3', '3333', '성3 이름3', 23, 1),
        ('user4', 'nickname4', '4444', '성4 이름4', 24, 1),
        ('user5', 'nickname5', '5555', '성5 이름5', 25, 1);

SELECT `user_id`, `nickname`, `password`, `name`, `age`, `grade`
	FROM `ssafit_db_v2`.`user`;

-- -----------------------------------------------------
-- Table `youtube_db`.`review`
-- -----------------------------------------------------
INSERT 
	INTO `ssafit_db_v2`.`review`
...
SELECT `review`.`review_no`, `review`.`content`, `review`.`date`, `video`.`title`, `user`.`name`
	FROM `ssafit_db_v2`.`review`
		LEFT OUTER JOIN `ssafit_db_v2`.`video` ON `review`.`video_id` = `video`.`video_id`
		LEFT OUTER JOIN `ssafit_db_v2`.`user` ON `review`.`user_id` = `user`.`user_id`;


-- -----------------------------------------------------
-- Table `youtube_db`.`follow`
-- -----------------------------------------------------
INSERT INTO `ssafit_db_v2`.`follow`
	VALUES
...
SELECT `user`.`name`, `channel`.`channel_name`
 	FROM `ssafit_db_v2`.`follow` 
 		LEFT OUTER JOIN `ssafit_db_v2`.`user` ON `follow`.`user_id` = `user`.`user_id`
 		LEFT OUTER JOIN `ssafit_db_v2`.`channel` ON `follow`.`channel_id` = `channel`.`channel_id`;


-- -----------------------------------------------------
-- Table `youtube_db_db`.`mark`
-- -----------------------------------------------------
INSERT INTO `ssafit_db_v2`.`mark`
	VALUES
		('user1', 'GhIpFWzuliA'),
        ('user2', 'nz5qsvRgK24'),
        ('user3', 'cMkZ6A7wngk'),
...
SELECT `user`.`name`, `video`.`title`
	FROM `ssafit_db_v2`.`mark` 
		LEFT OUTER JOIN `ssafit_db_v2`.`user` ON `mark`.`user_id` = `user`.`user_id`
		LEFT OUTER JOIN `ssafit_db_v2`.`video` ON `mark`.`video_id` = `video`.`video_id`;
        
-- -----------------------------------------------------
-- Table `youtube_db`.`tag_has_video`
-- -----------------------------------------------------
INSERT INTO `ssafit_db_v2`.`tag_has_video`
	VALUES
...
FROM `ssafit_db_v2`.`tag_has_video`;