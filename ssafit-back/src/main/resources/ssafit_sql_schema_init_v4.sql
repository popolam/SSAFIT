...
-- Schema ssafit_db_v4
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `ssafit_db_v4` ;
CREATE SCHEMA IF NOT EXISTS `ssafit_db_v4` DEFAULT CHARACTER SET utf8 ;
USE `ssafit_db_v4` ;

-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`channel`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`channel`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`channel` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`video`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`video`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`video` (
  ...
  CONSTRAINT `fk_video_channel1`
    ...
    REFERENCES `ssafit_db_v4`.`channel` (`channel_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`tag`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`tag`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`tag` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`tag_has_video`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`tag_has_video`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`tag_has_video` (
  `tag_name` VARCHAR(100) NOT NULL,
  `video_id` VARCHAR(45) NOT NULL,
  ...
  CONSTRAINT `fk_tag_has_video_tag`
    ...
    REFERENCES `ssafit_db_v4`.`tag` (`tag_name`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_tag_has_video_video1`
    FOREIGN KEY (`video_id`)
    REFERENCES `ssafit_db_v4`.`video` (`video_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`area`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`area`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`area` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`user`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`user`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`user` (
  ...
  CONSTRAINT `fk_user_area1`
    ...
    REFERENCES `ssafit_db_v4`.`area` (`area_name` , `class_num`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`mark`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`mark`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`mark` (
  `user_id` VARCHAR(45) NOT NULL,
  `video_id` VARCHAR(45) NOT NULL,
  ...
  CONSTRAINT `fk_mark_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `ssafit_db_v4`.`user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`follow`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`follow`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`follow` (
  `channel_id` VARCHAR(45) NOT NULL,
  `user_id` VARCHAR(45) NOT NULL,
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`review`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`review`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`review` (
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`score`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`score`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`score` (
  `user_id` VARCHAR(45) NOT NULL,
...
-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`file`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ssafit_db_v4`.`file`;
CREATE TABLE IF NOT EXISTS `ssafit_db_v4`.`file` (
  `user_id` VARCHAR(45) NOT NULL,
...
SELECT * FROM `ssafit_db_v4`.`channel`;
...
SELECT `area_name`, `class_num`
	FROM `ssafit_db_v4`.`area`;

-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`user`
-- -----------------------------------------------------
INSERT
	INTO `ssafit_db_v4`.`user` 
		(`user_id`, `nickname`, `password`, `name`, `age`, `grade`, `area_name`, `class_num`)
	VALUES 
		('admin@ssafy.com', '관리자', 'admin', '관리자', 0, 0, '서울', 1),
		('user1@ssafy.com', 'nickname1', '1111', '성1 이름1', 21, 1, '서울', 1),
        ('user2@ssafy.com', 'nickname2', '2222', '성2 이름2', 22, 1, '대전', 4),
        ('user3@ssafy.com', 'nickname3', '3333', '성3 이름3', 23, 1, '구미', 3),
        ('user4@ssafy.com', 'nickname4', '4444', '성4 이름4', 24, 1, '광주', 2),
        ('user5@ssafy.com', 'nickname5', '5555', '성5 이름5', 25, 1, '부울경', 1);

SELECT `user_id`, `nickname`, `password`, `name`, `age`, `grade`, `area_name`, `class_num`
	FROM `ssafit_db_v4`.`user`;

-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`file`
-- -----------------------------------------------------
INSERT
	INTO `ssafit_db_v4`.`file` 
		(`user_id`)
	VALUES 
		('admin@ssafy.com'),
		('user1@ssafy.com'),
...
SELECT `user_id`, `file_uri`, `file_name`
	FROM `ssafit_db_v4`.`file`;

-- -----------------------------------------------------
-- Table `ssafit_db_v4`.`score`
-- -----------------------------------------------------
-- INSERT
-- 	INTO `ssafit_db_v4`.`score` 
...
-- -----------------------------------------------------
INSERT 
	INTO `ssafit_db_v4`.`review`
		(`content`, `video_id`, `user_id`)
	VALUES
        ('리뷰 내용1', '2swcod5RYvU', 'user1@ssafy.com'),
        ('리뷰 내용2', '3TyTGxBNwic', 'user1@ssafy.com'),
        ('리뷰 내용3', '4EcIZexNa7A', 'user1@ssafy.com'),
        ('리뷰 내용4', '54tTYO-vU2E', 'user1@ssafy.com'),
...
SELECT `review`.`review_no`, `review`.`content`, `review`.`date`, `video`.`title`, `user`.`name`
	FROM `ssafit_db_v4`.`review`
		LEFT OUTER JOIN `ssafit_db_v4`.`video` ON `review`.`video_id` = `video`.`video_id`
		LEFT OUTER JOIN `ssafit_db_v4`.`user` ON `review`.`user_id` = `user`.`user_id`;
...
SELECT `follow`.`user_id`, `follow`.`channel_id`
 	FROM `ssafit_db_v4`.`follow` 
 		LEFT OUTER JOIN `ssafit_db_v4`.`user` ON `follow`.`user_id` = `user`.`user_id`
 		LEFT OUTER JOIN `ssafit_db_v4`.`channel` ON `follow`.`channel_id` = `channel`.`channel_id`;


-- -----------------------------------------------------
-- Table `youtube_db_db`.`mark`
-- -----------------------------------------------------
INSERT INTO `ssafit_db_v4`.`mark`
	VALUES
...
SELECT `user`.`name`, `video`.`title`
	FROM `ssafit_db_v4`.`mark` 
		LEFT OUTER JOIN `ssafit_db_v4`.`user` ON `mark`.`user_id` = `user`.`user_id`
		LEFT OUTER JOIN `ssafit_db_v4`.`video` ON `mark`.`video_id` = `video`.`video_id`;
        
SELECT `user`.`name`, `video`.`title`
	FROM `ssafit_db_v4`.`mark` 
		LEFT JOIN `ssafit_db_v4`.`user` ON `mark`.`user_id` = `user`.`user_id`
		LEFT JOIN `ssafit_db_v4`.`video` ON `mark`.`video_id` = `video`.`video_id`;
...
-- -----------------------------------------------------
INSERT INTO `ssafit_db_v4`.`tag_has_video`
	VALUES
...
FROM `ssafit_db_v4`.`tag_has_video`;